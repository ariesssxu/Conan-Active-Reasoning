...
class MC_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.mc):
            ai = config["choices"][i]
            text.append(self._get_text(subs, ai, question))
...
def build_mc_dataset(dataset_path, split):
    config_path = os.path.join(dataset_path, f"{split}/config.json")
    feature_path = os.path.join(dataset_path, f"{split}")
    subtitles_path = None
    max_feats = 30
    features_dim = 4096
    return MC_Dataset(
