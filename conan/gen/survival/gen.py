# survival agent gen. Initialize as an navigate agent.
...
from conan.gen.survival.agents import SurvivalAgent
...
def survival_check(env):
    return False

def gen_survival_single(env, args, size, id):

    env.reset()
    duration = 0

    print(f"----- Gen survival trace {id} -----")

    mat_map, obj_map = env.env.get_detailed_view()
    save_path = os.path.join(args.save_path, f"survival_{id}")
    env.set_directory(save_path)

    agent = SurvivalAgent(env.player.pos, mat_map, obj_map, env._player, env.player.inventory)
    ...
    while running:
        ...
        finished = survival_check(env.env)
        duration += 1
...
def gen_survivals(args):
    ...
    for finished in trange(0, args.num):
        ...
        env.reset()
        mat_map, obj_map = env.env.get_detailed_view()
        gen_survival_single(env, args, size, finished)
...
def main():
    ...
    parser.add_argument('--save_path', type=str, default="save/survival/")
    ...
    # remove other agents (skeleton, zombie) so that the player will not be hurt
