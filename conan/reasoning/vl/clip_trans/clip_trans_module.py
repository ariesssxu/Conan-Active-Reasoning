...
class CLIPTransWOTextEncoder(pl.LightningModule):
    ...
    def forward(self, images, texts):
        ...
        '''
        batch_size = images.shape[0]
        texts_input = einops.rearrange(texts, 'b c d -> (b c) d', c=1)

        image_features = einops.rearrange(
            images, 'b c h w -> b c (h w)')  # (batch_size, 30, 4096)
        image_features = torch.reshape(image_features, (batch_size, -1, 512))
        text_features = self.token_embedding(texts_input) # (batch_size, 11, 512)


        # inputs like cls, image_1, text_1s, sep, image_2, text_2s, sep, ..., image_6, sep, text 6s, text 7s, ..., text 11s
        image_contexts = image_features[:, :, :]  # (batch_size, -1, 512)
        # (batch_size, 6, self.token_length, 512)
        text_contexts = text_features

        # context_inputs should be (batch_size, 6, 1 + self.token_length + 1, d_model)
        context_inputs = torch.cat([image_contexts, text_contexts], dim=1)  # (batch_size, 1, 304, d_model)
        # context_inputs = einops.rearrange(
        #     context_inputs, 'b c l d -> b (c l) d')  # (batch_size, 6 * (self.token_length + 2), d_model)

        # transformer_inputs = einops.rearrange(context_inputs, '(b c) t d -> b c t d', b=batch_size, c=1)
        transformer_inputs = context_inputs
        transformer_outputs = self.transformer(transformer_inputs)
