...
from flamingo_mini import FlamingoModelForMultipleChoice, FlamingoConfig, FlamingoTokenizer, FlamingoHEAD
...
def main():
    ...
    # raw_datasets = {split: LexiconDatasetForFlamingo(data_args.dataset_path, split=split, task_name=data_args.task_name) for split in ["train", "val", "test"]}
    train_dataset = build_mc_dataset(data_args.dataset_path, split="train")
    eval_dataset = build_mc_dataset(data_args.dataset_path, split="val")
    ...
    config.pad_token_id = GPT2_tokenizer.eos_token_id
    if data_args.pretrained:
        if model_args.cnn_head:
            # model = FlamingoHEAD(
            #     config, 
            #     model_args.model_name
            # )
            model = FlamingoHEAD(config)
            model.load_state_dict(torch.load(model_args.model_name + "/pytorch_model.bin"))
            model.freeze_flamingo()
        ...
        if not model_args.cnn_head:
            model = FlamingoModelForMultipleChoice(config)
        else:
            model = FlamingoHEAD(
                config, 
                None
